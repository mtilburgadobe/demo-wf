...
Before running the full check, probe the first URL to detect htaccess protection:
...
- If the status is **401**: the site is htaccess-protected. Ask the user:
  > "The site returned 401 (HTTP Basic Auth required). Please provide credentials in `user:password` format."
  Then run Step 5 with `--auth=user:password`.
...
The script:
- Maps each sitemap URL path onto the EDS base domain
